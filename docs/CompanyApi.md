...
with invoicetronic_sdk.ApiClient(configuration) as api_client:
    ...
    try:
        # Get a company by vat number
        api_response = api_instance.company_vat_get(vat)
        print("The response of CompanyApi->company_vat_get:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling CompanyApi->company_vat_get: %s\n" % e)
```
